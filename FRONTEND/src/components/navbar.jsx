...
export default function Navbar({ title, userRole, userName, onLogout }) {
  const [showDropdown, setShowDropdown] = useState(false)

  const roleBadge = userRole === 'owner'
    ? { label: 'Owner', bg: 'bg-indigo-100 text-indigo-700' }
    : { label: 'Employee', bg: 'bg-emerald-100 text-emerald-700' }

  const avatarBg = userRole === 'owner' ? 'bg-indigo-500' : 'bg-emerald-500'
  ...
  return (
    <header className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between z-10 shrink-0">
      ...
      <div className="flex items-center gap-3">
        ...
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${roleBadge.bg}`}>
          {roleBadge.label}
        </span>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          ...
          >
            <div className={`w-8 h-8 rounded-full ${avatarBg} flex items-center justify-center text-sm font-bold text-white`}>
              {initial}
            </div>
            <div className="hidden md:block text-left">
              <p className="text-sm font-medium text-slate-700 leading-tight">{userName || 'User'}</p>
              <p className="text-xs text-slate-400">{roleBadge.label}</p>
            </div>
